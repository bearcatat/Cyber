...
#define CLEAR_COLOR "\033[0m"
    ...
    LogContextCapturer::LogContextCapturer(Logger &logger, LogLevel level, const char *file, const char *function, int line, std::thread::id thread_id)
        : ctx_(new LogContext(level, file, function, line, std::move(thread_id))), logger_(logger)
    {
    }

    LogContextCapturer::LogContextCapturer(const LogContextCapturer &that) : ctx_(that.ctx_), logger_(that.logger_)
    {
    ...
    {
        {
            ...
            pending_.emplace_back(std::make_pair(ctx, &logger));
        }
        sem_.Post();
    }
    ...
    LogChannel::LogChannel(const std::string &name, LogLevel level) : name_(name), level_(level) {}
    ...
    void ConsoleChannel::Write(const Logger &logger, const LogContextPtr &log_context)
    {
        ...
        format(logger, std::cout, log_context);
    }
} // namespace cyberweb
