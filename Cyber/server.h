...
namespace cyber
{
    ...
    public:
        ...
        {
            static std::shared_ptr<SessionMap> s_instance = std::make_shared();
        ...
        void ForEachSession(const std::function<void(cosnt string &id, const Session::Ptr &session)> cb)
        {
            LOCKGUARD(mtx_session_);
    ...
    public:
        ...
        SessionHelper(cosnt std::weak_ptr<Server> &server, Session::Ptr session)
        {
            server_ = server;
            session_ = session;
    ...
    public:
        ...
        {
            poller_ = poller ? poller : EventPoller::Instance().GetPoller();
        }
        virtual ~Server(){};
    };
